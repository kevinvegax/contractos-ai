...
type OverviewTask = { id: string; title: string; status: 'open' | 'in_progress' | 'blocked' | 'done'; due_date: string | null }
type OverviewEvidence = { id: string; name: string; storage_key: string; created_at: string }
type ProjectOverview = { project: Project; tasks: OverviewTask[]; evidence: OverviewEvidence[]; metrics: { total_tasks: number; completed_tasks: number; open_tasks: number; overdue_tasks: number; blocked_tasks: number; completion_percentage: number } }
...
function App() {
  ...
  const [taskFilter, setTaskFilter] = useState<'all' | 'overdue' | 'blocked' | 'incomplete'>('all')
  ...
  async function openProjectOverview(projectId: string) {
    setOverviewBusy(true); setOverviewError(''); setProjectOverview(null); setTaskFilter('all')
    try {
      const response = await fetch(`/api/projects?company_id=${workspace.id}&id=${projectId}`)
      const payload = await response.json() as ProjectOverview & { error?: string }
      if (!response.ok) { setOverviewError(payload.error ?? 'Unable to load project overview.'); return }
      setProjectOverview(payload)
    } catch { setOverviewError('The project service is unavailable. Please try again.') } finally { setOverviewBusy(false) }
  }

  return <div className="app-shell">
    {active === 'Projects' && <section className="projects-directory"><div className="directory-heading"><div><p className="kicker">Workspace</p><h1>All projects</h1><p>Every project in {workspace.name}.</p></div>{canCreateProject && <button className="primary-action" onClick={() => { setEditingProject(null); setProjectMessage(''); setShowProjectForm(true) }}><Icon name="plus" /> New project</button>}</div><div className="directory-grid">{projects.map((project) => <article className="directory-card" key={project.id}><div className="directory-card-top"><span className="project-mark">{project.name.slice(0, 1)}</span><span className={`pill ${project.status}`}><i /> {project.status === 'on_hold' ? 'On Hold' : project.status.charAt(0).toUpperCase() + project.status.slice(1)}</span></div><h2>{project.name}</h2><p>{project.description || 'No description added yet.'}</p><div className="directory-dates"><span>Start <strong>{project.start_date || 'Not set'}</strong></span><span>Due <strong>{project.due_date || 'Not set'}</strong></span></div><div className="directory-actions"><button type="button" onClick={() => void openProjectOverview(project.id)}>View overview</button>{canCreateProject && project.status !== 'archived' && <button type="button" onClick={() => { setEditingProject(project); setProjectMessage(''); setShowProjectForm(true) }}>Edit</button>}</div></article>)}{projects.length === 0 && <p className="overview-state">No projects have been created yet.</p>}</div></section>}
    ...
    <main className="main-content"><header className="topbar"><div className="breadcrumb"><span>Workspace</span><b>/</b><strong>{active}</strong></div><div className="top-actions"><span className="status-dot" /> All systems operational <button className="help">?</button><span className="top-avatar">{initials}</span></div></header>
      <div className="page-wrap"><section className="welcome"><div><p className="kicker">{new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p><h1>Good morning, Maya<span className="wave">✦</span></h1><p className="subhead">Here’s what’s happening across your company workspace.</p></div>{canCreateProject && <button className="primary-action" onClick={() => { setEditingProject(null); setProjectMessage(''); setShowProjectForm(true) }}><Icon name="plus" /> New project</button>}</section>
        ...
        {(overviewBusy || overviewError || projectOverview) && <div className="modal-backdrop"><section className="project-modal overview-modal" role="dialog" aria-modal="true" aria-labelledby="project-overview-title"><div className="modal-heading"><div><p className="kicker">Project overview</p>{projectOverview ? <><h2 id="project-overview-title">{projectOverview.project.name}</h2><p>{projectOverview.project.description || 'No project description yet.'}</p></> : <h2 id="project-overview-title">Loading project overview</h2>}</div><button className="modal-close" type="button" aria-label="Close" onClick={() => setProjectOverview(null)}>×</button></div>{overviewBusy && <p className="overview-state">Loading tasks and evidence…</p>}{overviewError && <div className="signin-error" role="alert">{overviewError}</div>}{projectOverview && <><div className="overview-meta"><span className={`pill ${projectOverview.project.status}`}><i /> {projectOverview.project.status === 'on_hold' ? 'On Hold' : projectOverview.project.status.charAt(0).toUpperCase() + projectOverview.project.status.slice(1)}</span><span>Start {projectOverview.project.start_date || 'Not set'}</span><span>Due {projectOverview.project.due_date || 'Not set'}</span></div><p className="overview-objectives"><strong>Objectives</strong>{projectOverview.project.objectives || 'No objectives defined yet.'}</p><div className="overview-metrics"><button type="button" onClick={() => setTaskFilter('all')}><strong>{projectOverview.metrics.total_tasks}</strong><span>Total tasks</span></button><button type="button" onClick={() => setTaskFilter('all')}><strong>{projectOverview.metrics.completion_percentage}%</strong><span>Complete</span></button><button type="button" className={projectOverview.metrics.overdue_tasks ? 'risk' : ''} onClick={() => setTaskFilter('overdue')}><strong>{projectOverview.metrics.overdue_tasks}</strong><span>Overdue</span></button><button type="button" className={projectOverview.metrics.blocked_tasks ? 'risk' : ''} onClick={() => setTaskFilter('blocked')}><strong>{projectOverview.metrics.blocked_tasks}</strong><span>Blocked</span></button></div><div className="overview-section" id="overview-tasks"><div className="overview-section-heading"><h3>Tasks</h3><button type="button" onClick={() => setTaskFilter('incomplete')}>Show incomplete ({projectOverview.metrics.open_tasks})</button></div>{projectOverview.tasks.filter((task) => taskFilter === 'all' || (taskFilter === 'overdue' && task.due_date && task.due_date < new Date().toISOString().slice(0, 10) && task.status !== 'done') || (taskFilter === 'blocked' && task.status === 'blocked') || (taskFilter === 'incomplete' && task.status !== 'done')).map((task) => <div className="overview-task" key={task.id}><span><strong>{task.title}</strong><small>{task.due_date ? `Due ${task.due_date}` : 'No due date'}</small></span><span className={`task-status ${task.status}`}>{task.status === 'in_progress' ? 'In progress' : task.status === 'blocked' ? 'Blocked' : task.status === 'done' ? 'Done' : 'Open'}</span></div>)}{projectOverview.tasks.filter((task) => taskFilter === 'all' || (taskFilter === 'overdue' && task.due_date && task.due_date < new Date().toISOString().slice(0, 10) && task.status !== 'done') || (taskFilter === 'blocked' && task.status === 'blocked') || (taskFilter === 'incomplete' && task.status !== 'done')).length === 0 && <p className="overview-state">No tasks match this view.</p>}</div><div className="overview-section"><h3>Evidence</h3>{projectOverview.evidence.length ? projectOverview.evidence.map((file) => <div className="overview-evidence" key={file.id}><span>{file.name}</span><small>Added {new Date(file.created_at).toLocaleDateString()}</small></div>) : <p className="overview-state">No evidence files attached yet.</p>}</div></>}</section></div>}
